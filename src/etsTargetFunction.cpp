...
void EtsTargetFunction::init(std::vector<double> & p_y, int p_errortype,
...
void EtsTargetFunction::etscalc(){
  int i, j;
...
void EtsTargetFunction::forecast(double& l,  double& b, double *s, double *f, int& h)
{
    int i,j;
    double phistar;
...
void EtsTargetFunction::update(double* oldl, double* l, double* oldb, double* b, double *olds, double *s, double& y)
{
    int j;
    double q, p, r, t, phib;
...
void EtsTargetFunction::oneEval(std::vector<double> & p_y, int p_errortype,
  	int p_trendtype, int p_seasontype, bool p_damped,
		int p_nmse, int p_m,
...
double EtsTargetFunction::rhobiweight(double x){
  double rho = 1.0;
  if(std::abs(x)<k){
...
double EtsTargetFunction::psihuber(double x){
  if(std::abs(x)<k){
    return x;
  }else{
    return copysign(k,x);
...
double EtsTargetFunction::median(std::vector<double> x) {
...
double EtsTargetFunction::roblikM(std::vector<double>& x){
  // compute Erho
